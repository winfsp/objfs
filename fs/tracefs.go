...
func (self *TraceFs) Readdir(path string,
	...
	ofst int64,
	fh uint64) (errc int) {
	defer traceFuse(path, ofst, fh)(&errc)
...
func (self *TraceFs) Setxattr(path string, name string, value []byte, flags int) (errc int) {
	defer traceFuse(path, name, value, flags)(&errc)
